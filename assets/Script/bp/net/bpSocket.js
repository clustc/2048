...
var ws = {
    ...
    initProtoBuf: function (path, protoModelNameMap) {
        ...
        cc.loader.load(url, (error, data)=>{
            ...
            if(!error){
                ...
                protobuf.parse(data, root);
                ProtocolData = root.lookupType("ProtocolData");
                for (let key in protoModelNameMap) {
                    let modelName = protoModelNameMap[key];
                    if(modelName != ""){
                        self.protoModelMap["" + key] = root.lookupType(modelName);
                    }
                }
                this._protoFileLoaded = true;
                bp.event.emit("ProtoInited");
            }
        });
    },
